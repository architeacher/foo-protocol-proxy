...
TARGET_IMAGE ?= $(REGISTRY_REPO):$(DOCKER_IMAGE_TAG)
...
docker-shell: .env build-dirs ## 🐳 run shell command inside the docker container, Example: make docker-shell CMD="-c 'ls > .files'"
	@echo "$(WARN_CLR)$(MSG_PRFX) ${DOCKER_LOGO} 🏃 running a image \"$(BUILD_IMAGE)\" shell in the containerized build environment$(MSG_SFX)$(NO_CLR)"
	@$(DOCKER) run                                   \
	    -ti                                          \
	    --rm                                         \
	    -u $$(id -u):$$(id -g)                       \
...
image: .dockerfile-$(ARCH) ## 🐳 to build a docker image.
	@echo "$(INFO_CLR)$(MSG_PRFX) ${DOCKER_LOGO} 🌄 Creating image \"$(TARGET_IMAGE)\"$(MSG_SFX)$(NO_CLR)"
	@$(DOCKER) build          \
	    ${DOCKER_BUILD_FLAGS} \
	    -t $(TARGET_IMAGE)    \
	    -f $(DOCKER_FILE)     \
...
publish: ## 🐳 to publish the docker image to dockerhub repository.
	@echo "$(WARN_CLR)$(MSG_PRFX) ${DOCKER_LOGO} 🚀 Pushing image to \"$(TARGET_IMAGE)$\"(MSG_SFX)$(NO_CLR)"
	@$(DOCKER) push $(TARGET_IMAGE) 2>&1
